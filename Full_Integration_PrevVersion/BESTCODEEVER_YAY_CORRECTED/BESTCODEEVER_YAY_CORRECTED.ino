      ...
      if(CheckWideRangeForServer()){ //check if looking at a server beacon
        ...
        switch(orientation){
          ...
          case(RIGHT_ORIENTATION) :
            Stop();
            delay(PAUSE_MILLIS);
            Serial.println("Bot in right orientation and facing towards server ");
...
// break;
          ...
          case(LEFT_ORIENTATION) :
            ...
            delay(PAUSE_MILLIS);
            Serial.println("Bot in left orientation and facing towards server - ");
         ...
         // if (orientation == LEFT_ORIENTATION){ SetMotors
            if(CheckWideRangeForServer() == true){
              if(orientation == LEFT_ORIENTATION){
                SpinLeft(SCANNING_SPEED);
                delay(200); // Delay to make this state more visible
              }
              Stop();
              delay(PAUSE_MILLIS);
              Serial.println("Displacing");
              ChangeState(DISPLACING); //stopping sequence
            }
          
          break;
    case(CORRECTING_SHORT_RANGE):
       if(CheckShortRangeForSignal() == true){ // use cshort range sensor to correct back to server
//              if(orientation == RIGHT_ORIENTATION){
...
void DriveForwardCorrected(char newSpeed){ // REVERSED
