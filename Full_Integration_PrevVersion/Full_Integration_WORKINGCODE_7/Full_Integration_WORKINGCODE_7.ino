...
#define BUTTON_PRESS_MILLIS 150
#define BUTTON_PRESS_WAIT_MILLIS 600 // time of delay that bot is waiting at pushed button before leaving
#define PAUSE_MILLIS 1000
#define DUMP_DELAY_MILLIS 2500
#define BUMPER_MILLIS 2000
...
int coinMax = 4; // total number of button presses needed to get desired coins (2 coins = 3 presses)
...
//        TMRArd_InitTimer(8, PAUSE_MILLIS); /// UPDATE 6
//        if(CheckWideRangeForServer() == true && TMRArd_IsTimerExpired(8)) {
              Stop();
...
//              ChangeState(DISPLACING);
    ...
    case(CORRECTING_SHORT_RANGE):
       if(CheckShortRangeForSignal() == true){ // use cshort range sensor to correct back to server
              Stop();
          if (orientation == LEFT_ORIENTATION){ // UPDATED TOO
          SpinRight(SCANNING_SPEED_2); /// UPDATED
          delay(50);
          Stop();
       }
       else{
         SpinLeft(SCANNING_SPEED_2);
         delay(50);
         Stop();
       }  /// END UPDATE
       break;
...
//            break;
    case(SEARCHING_FOR_SERVER_SHORT) :
      if(CheckShortRangeForSignal() == false){ // try checking wide range for server as well if short gives problems
        /*if(orientation == LEFT_ORIENTATION){
SpinRight(SCANNING_SPEED);
delay(100);
}
*/
        Stop();
//        delay(PAUSE_MILLIS); //CHANGED HERE 
...
case(ADJUSTING):
     ...
     if(LBB != RBB){ // if bumpers are not the same, then only one side hit wall
      ...
      if((LBB == BUMPEROPEN) && (RBB == BUMPERHIT) && TMRArd_IsTimerExpired(7) == false) { // right back hit, left back not, need to twist left towards wall
      LeftMtrSpeed(-1 * TRAVELING_SPEED);
      RightMtrSpeed(0);
      }
      else if ((LBB == BUMPERHIT) && (RBB == BUMPEROPEN) && TMRArd_IsTimerExpired(7) == false) { // left back hit, right back not, need to twist right towards wall
  ...
  case(MOVING_TOWARDS_EXCHANGE) :
    CheckBumpers();
    if ((LFB == BUMPERHIT) || (RFB == BUMPERHIT) && (dumped == 0)) { //both rear bumpers depressed, stop motors CHANGE: OR not AND
      dumped = 1;
...
//      CheckBumpers();
//      if(LFB == BUMPERHIT || RFB == BUMPERHIT || LBB == BUMPERHIT || RBB == BUMPERHIT) {
//        Stop();
//         TMRArd_InitTimer(WIDE_RANGE_NO_SIGNAL_TIMER, WIDE_RANGE_NO_SIGNAL_MILLIS);//millis
//        TMRArd_InitTimer(SHORT_RANGE_BEACON_SIGNAL_TIMER , SHORT_RANGE_NO_SIGNAL_MILLIS);
//        ChangeState(SEARCHING_FOR_SERVER_WIDE);
...
void SetMotors(int newState){
   ...
   switch(newState) {
     ...
     case(CORRECTING_SHORT_RANGE):
       if (orientation == LEFT_ORIENTATION){
          Serial.println("SHORT RANGE -correcting towards the LEFT");
          SpinRight(SCANNING_SPEED_2); /// UPDATED
       }
       else{
...
void DriveBackwardCorrected(char newSpeed){ // Ideal speed in testing was Right Motor 7, left motor 9
...
LeftMtrSpeed(-1 * (newSpeed+1));  // UPDATE 3
RightMtrSpeed(1 * (newSpeed-1)); // UPDATE 3
}

void SpinLeft(char newSpeed){
LeftMtrSpeed(1 * (newSpeed+1));   // UPDATE 2
RightMtrSpeed(-1 * (newSpeed-1));  // UPDATE 2
}
